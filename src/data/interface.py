...
class LitData(pl.LightningDataModule):
    ...
    def setup(self, stage):
        ...
        if stage == "fit" or self.needs_train_info:
            self.train_dset, _ = self.split_each(
                self.images, self.normals, None, self.i_train, dummy=False, _images_GAN=self.images2,
            )
            self.train_image_sizes = self.image_sizes[self.i_train]

        if stage == "fit" or stage is None:
            self.val_dset, self.val_dummy = self.split_each(
                self.images, self.normals, None, self.i_val, dummy=True, _images_GAN=self.images2,
            )
        ...
        if stage == "test" or stage is None:
            ...
            self.test_dset, self.test_dummy = self.split_each(
                self.images, self.normals, None, test_idx, dummy=True, _images_GAN=self.images2,
            )
    ...
    def split_each(
        ...
        if _images_GAN is not None:
            # print(colored("not none", 'red'))
            images_idx = np.concatenate([_images_GAN[i].reshape(-1, 3) for i in idx])
            images_GAN = np.zeros((n_dset + dummy_num, 3))
            images_GAN[:n_dset] = images_idx
            images_GAN[n_dset:] = images_GAN[:dummy_num]
...
class RaySet(Dataset):
    def __init__(self, rays_info):
        ...
        self.images_GAN = rays_info["images_GAN"]
    ...
    def __getitem__(self, index):
        ...
        ret["targetGAN"] = np.zeros_like(ret["rays_o"])

        if self.images is not None:
            ret["target"] = torch.from_numpy(self.images[index])
            ret["targetGAN"] = torch.from_numpy(self.images_GAN[index])
