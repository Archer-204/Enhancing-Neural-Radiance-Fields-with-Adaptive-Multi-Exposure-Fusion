...
class NeRF(nn.Module):
    def __init__(
            self,
            num_levels: int = 2,  # coarse_mlp and fine_mlp
            min_deg_point: int = 0,
    ...
    def forward(self, rays, randomized, white_bkgd, near, far):
        ...
        for i_level in range(self.num_levels):
            ...
            rgb = self.rgb_activation(raw_rgb)  # nn.Sigmoid()
            sigma = self.sigma_activation(raw_sigma)  # nn.ReLU()

            comp_rgb, depth, acc, weights = helper.volumetric_rendering(
                rgb,
                sigma,
                t_vals,
                rays["rays_d"],
...
class LitNeRF(LitModel):
    def __init__(
        ...
        self.model = NeRF()  # put NERF on the optimizer
    ...
    def validation_step(self, batch, batch_idx):
        output = self.render_rays(batch, batch_idx)
    ...
    def on_validation_epoch_end(self):
        ...
        rgbs = self.alter_gather_cat(self.validation_step_outputs, "rgb", val_image_sizes)
        targets = self.alter_gather_cat(self.validation_step_outputs, "target", val_image_sizes)
    ...
    def on_test_epoch_end(self):
        ...
        rgbs = self.alter_gather_cat(self.test_step_outputs, "rgb", all_image_sizes)
        depths = self.alter_gather_cat(self.test_step_outputs, "depth", all_image_sizes)
        targets = self.alter_gather_cat(self.test_step_outputs, "target", all_image_sizes)
